...
## 2026-08-18 — 预览框直接编辑画面属性

**完成**

- 主图标题、副标题和画幅比例从 Side Panel 表单移除，侧边栏聚焦 AI 输入、出行方式和路径点管理。
- Google Maps 画幅框顶部新增可直接编辑的标题、副标题输入框，以及 3:4、4:5、9:16 比例按钮。
- 预览框修改通过扩展内部消息同步回 Side Panel 的本地项目状态，最终导入 Web App 和高清导出使用相同值。
- 画幅框主体保持地图可观察，只有顶部编辑区响应鼠标操作。

**验证**

- Manifest 通过 JSON 解析，扩展脚本通过 Node 语法检查。
- `npm run lint` 与 `npm run build` 通过；保留现有包体积提示。

---

## 2026-08-18 — 分段出行方式与自适应视野

**完成**

- Side Panel 移除全局出行方式；第一个路径点显示为起点，之后每点在展开卡片中设置“前往此点：步行/驾车”，折叠态显示到达方式。
- Place 与插件导入契约新增 `arrivalMode`；排序后重新规范首站与后续站点的到达方式。
- 新增 mixed route 聚合服务：单一方式保持一次多点请求，混合方式按相邻路段并行计算并合并 geometry、距离、时间和 legs。
- 预览桥接返回分段 geometry；Google Maps 画幅中步行段显示蓝色虚线，驾车段显示青绿色实线。
- Web App 的 Konva 高清导出同步显示分段颜色，逐段时间气泡和右侧列表标明步行或驾车。
- 更新预览时根据所有路径点边界计算中心和缩放；单一方式打开 Google 原生多点导航，混合方式打开自适应地图视野并叠加准确分段路线。
...
- 混合出行需要按方式切换点拆分路线请求，API 调用次数可能高于统一步行或统一驾车路线；仍由用户点击“更新真实路线预览”时触发。

---

## 2026-08-18 — 画幅级自适应缩放与平移

**完成**

- 替换基于经纬度跨度的粗略 zoom 估算，改由 Google Maps 内容脚本读取当前预览框真实像素尺寸。
- 使用 Web Mercator 计算路径点与完整 route geometry 的投影边界，并分别求得横向、纵向可容纳的最佳 zoom。
- fit 计算预留顶部标题区、底部路线摘要区和左右安全边距；通过调整相机中心纬度实现垂直平移补偿，使路径点落入实际可见区域。
- 点击更新路线后自动导航到计算出的中心与 zoom；切换 3:4、4:5、9:16 时自动重新 fit。
- 用户后续手动平移或缩放仍会触发叠加路线重绘。
...
- Google Maps 页面 URL 仍是相机状态同步依据；高清导出使用 Web App 自身的地图控制器与 Static Maps，不依赖扩展预览投影。

---

## 2026-08-18 — AI 路线编排、真实地点与地点照片

**完成**

- 新增服务端 AI 路线 JSON 契约及运行时校验；模型只生成标题、城市、路径点名称、顺序、到达方式和 120 字内说明，不接收模型生成的坐标或图片。
- Side Panel 的“规划为路径点”接通 Web App 桥接：AI 草案逐站经 Google Places 校准 Place ID、地址和坐标，全部成功后再调用 Google Routes 生成路线及分段时间。
- 整个流程采用原子更新；任一地点无法确认或路线无法生成时保留用户当前路径点。
- Places 搜索新增首张地点照片、Google Maps 图片来源和作者归属；路径点与副图导出保留归属信息。
- 新增 `VITE_AI_ROUTE_ENDPOINT`，模型密钥仅允许配置在服务端，不进入 Web App 或扩展包。
...
## 2026-08-18 — 预览框快照与极简 Static Map 导出

**完成**

- Google Maps 蓝色预览框顶部新增“📸 快照”按钮；点击后播放白色快门闪光和画幅轻微收缩特效。
- 新增服务端 `/api/google/static-map`：使用 Maps Static API 生成 3:4、4:5、9:16 底图，并隐藏全部 Google 底图标签、POI 和交通文字。
- 扩展在 Canvas 中叠加分段路线、编号、路径点名称、标题、副标题、距离、时间与 TripCanvas 水印，再通过 Chrome downloads 下载 PNG。
- 快照输出尺寸为 1080×1440、1080×1350、1080×1920；Static Map 保留 Google 官方品牌与法律归属。
- 实时预览中的编号点旁同步显示路径点名称，便于与快照结果对照。

**验证**

- Static Maps 真实请求返回 PNG Data URL，逻辑尺寸 480×640，图片数据约 226 KB。
- TypeScript、扩展脚本语法、Manifest JSON 和生产构建通过。
- 当前自动化浏览器未连接安装本地扩展的 Chrome，最终按钮点击、下载栏和快门动画需在重新加载扩展后手动确认。
